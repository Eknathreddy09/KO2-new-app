...
```bash
./mvnw package  
java -jar target/ko2-app-hello-0.0.1-SNAPSHOT.jar --app.message=Test
```
...
```bash
tanzu apps workload create ko2-app-hello -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/ko2-app-hello-source \
  --type web
```
...
```
tanzu apps workload get ko2-app-hello
```
